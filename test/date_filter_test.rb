...
class DateFilterTest < Minitest::Test
  # --- parse_cutoff: relative durations ---

  def test_parse_cutoff_days
    cutoff = Safe::DateFilter.parse_cutoff("7d")
    today = Date.today
    expected = Time.new(today.year, today.month, today.day) - (7 * 86_400)
  ...
  def test_parse_cutoff_zero_days
    ...
    today = Date.today
    expected = Time.new(today.year, today.month, today.day)
    assert_in_delta expected.to_f, cutoff.to_f, 1.0
  end

  def test_parse_cutoff_months
    cutoff = Safe::DateFilter.parse_cutoff("6m")
    expected_date = Date.today << 6
    expected = Time.new(expected_date.year, expected_date.month, expected_date.day)
    assert_equal expected, cutoff
  end

  def test_parse_cutoff_years
    cutoff = Safe::DateFilter.parse_cutoff("1y")
  ...
  def test_parse_cutoff_uppercase
    ...
    today = Date.today
    expected = Time.new(today.year, today.month, today.day) - (30 * 86_400)
    assert_in_delta expected.to_f, cutoff.to_f, 1.0
  end
  ...
  def test_parse_cutoff_large_days
    ...
    today = Date.today
    expected = Time.new(today.year, today.month, today.day) - (365 * 86_400)
    assert_in_delta expected.to_f, cutoff.to_f, 1.0
  end

  # --- parse_cutoff: absolute dates ---
  ...
  def test_parse_cutoff_months_from_jan_31
    cutoff = Safe::DateFilter.parse_cutoff("1m", reference_date: Date.new(2026, 1, 31))
  ...
  def test_parse_cutoff_months_from_mar_31
    cutoff = Safe::DateFilter.parse_cutoff("1m", reference_date: Date.new(2026, 3, 31))
  ...
  def test_parse_cutoff_leap_year
    cutoff = Safe::DateFilter.parse_cutoff("1y", reference_date: Date.new(2024, 2, 29))
    assert_equal 2, cutoff.month
  ...
  def test_safe_boundary_exact_match_is_not_safe
    # Exact match: publication == cutoff → not safe (must be strictly before)
